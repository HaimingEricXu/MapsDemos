    ...
    GMUClusterManagerDelegate
{
   ...
   private func lockedSnackbar() -> Bool {
    ...
    ///
    /// - Parameter rad: The radius of the circle, initially set to 2000, but changes depending on the zoom.
    private func radius(rad: Double = 2000) {
        ...
        overlayController.drawCircle(
            mapView: mapView,
            darkModeToggle: darkModeToggle,
            coord: mapsIdentifier.getCoord(),
            rad: rad
        )
        for _ in 0...5 {
            let lat = Double(Float(mapsIdentifier.getCoord().latitude)
                + Float.random(in: -Float((rad / radiusFactor))..<Float((rad / radiusFactor))))
            let long = Double(Float(mapsIdentifier.getCoord().longitude)
                + Float.random(in: -Float((rad / radiusFactor))..<Float((rad / radiusFactor))))
            overlayController.geocode(latitude: lat, longitude: long) { (placemark, error, pid) in
                DispatchQueue.main.async {
                    ...
                    if pid.count >= 4 {
                        ...
                        tempMarker.position = CLLocationCoordinate2D(
                            latitude: lat,
                            longitude: long
                        )
                        self.locationImageController.viewImage(
                            placeId: String(officialPid),
                            localMarker: tempMarker,
